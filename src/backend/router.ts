...
export function setErrorPage(template: HandlebarsTemplateDelegate): void {
	errorPage = (props) => {
...
function sendError(data: RouteErrorHandlerProps, sendHtml = true): Response {
...
export function post(uri: `/${string}`, content: RouteHandler): void {
	setSingleRoute("POST", uri, content);
}
export function del(uri: `/${string}`, content: RouteHandler): void {
	setSingleRoute("DELETE", uri, content);
}
export function put(uri: `/${string}`, content: RouteHandler): void {
	setSingleRoute("PUT", uri, content);
}
export function patch(uri: `/${string}`, content: RouteHandler): void {
	setSingleRoute("PATCH", uri, content);
}
export function any(uri: `/${string}`, content: RouteHandler): void {
	routes.push({
		uri: uri,
		accepts: {
			POST: true,
...
function methodIsInvalid(method: HttpMethod, handler: Route): boolean {
	return (
		(method == "GET" && !handler.accepts.GET) ||
...
export async function serve(request: Request): Promise<Response> {
	...
	const sendHtmlError = request.headers
		.get("Accept")
		?.includes(MimeType.Html);
	...
	try {
		const routePath = config.paths.resources + route;
